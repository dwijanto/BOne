...
Public Class FormAutoGenerateWarehouse
    ...
    End Sub
    Sub dowork()
        Logger.log("--------Start Warehouse----------")
        ProgressReport(6, "Start")
        Logger.log("Generate HK Warehouse Text File.")
        Dim HKTask As New HKWarehouse(Me)
        HKTask.LoadData(Date.Today.AddDays(-1))
        HKTask.ExportTextFile()
        If Not HKTask.DoExcelWork() Then
            Logger.log(String.Format("Error Found: {0}", HKTask.errorMsg))
        End If
        Logger.log("Generate TW Warehouse Text File.")
        Dim TWTask As New TWWarehouse(Me)
        TWTask.LoadData(Date.Today.AddDays(-1))
        TWTask.ExportTextFile()
        If Not TWTask.DoExcelWork() Then
            Logger.log(String.Format("Error Found: {0}", TWTask.errorMsg))
        End If
